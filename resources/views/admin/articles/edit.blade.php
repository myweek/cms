...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold">编辑文章</h1>
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                <input type="hidden" name="content" id="content-field" value="{{ old('content', $article->content) }}">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="category_id" id="category_id" required
                            ...
                            @foreach(\App\Models\Column::where('is_active', true)->orderBy('order')->get() as $column)
                                <option value="{{ $column->id }}" {{ old('category_id', $article->category_id) == $column->id ? 'selected' : '' }}>
                                    {{ $column->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-span-2">
                        ...
                        @if($article->cover_image)
                            <div class="mb-2">
                                <img src="{{ asset('storage/' . $article->cover_image) }}" alt="{{ $article->title }}" class="h-32 w-auto object-cover rounded">
                            </div>
                        @endif
                        <input type="file" name="cover_image" id="cover_image" accept="image/*"
                            class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
                        <p class="mt-1 text-sm text-gray-500">支持 JPG, PNG, GIF 格式图片，不上传则保持原图</p>
                    </div>

                    <div class="col-span-2">
                        <label for="excerpt" class="block text-sm font-medium text-gray-700 mb-1">文章摘要</label>
                        <textarea name="excerpt" id="excerpt" rows="3"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('excerpt', $article->excerpt) }}</textarea>
                    ...
                    <div class="col-span-2">
                        ...
                        <div id="editor-content" class="min-h-[300px] border border-gray-300 rounded-md">{!! old('content', $article->content) !!}</div>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            ...
                            <option value="pending" {{ old('status', $article->status) == 'pending' ? 'selected' : '' }}>待审核</option>
                        </select>
                    </div>
                    ...
                    <div class="col-span-2 flex space-x-6">
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" name="is_top" id="is_top" value="1" 
                                {{ old('is_top', $article->is_top) ? 'checked' : '' }}
                                class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
